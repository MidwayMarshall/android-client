...
public class SpectatingBattle {
	...
	*/
	...
	public void dealWithCommand(BattleCommand bc, byte num, Bais msg) {
		...
		switch(bc) {
			case SendOut: {
				Log.e(TAG, "num " + num + "  player " + player + "  slot " + slot);
			...
			if(msg.available() > 0) { // this is the first time you've seen it
				...
				Log.e(TAG, "Player " + player + " sent out " + pokes[player][slot].rnick);
				...
				boolean silent = msg.readBool();
				Log.e(TAG, "num " + num + "  player " + player + "  slot " + slot);
				Log.e(TAG, "Player " + player + "  used Move " + MoveInfo.name(attack));
				if (!silent) {
			...
			*/
            ...
            /* Balloon gets a really special treatment */
				...
				Log.e(TAG, "num " + num + "  player " + player + "  slot " + slot);
				Log.e(TAG, "Damage " + damage + "  player " + player + "'s " + pokes[player][slot].nick);
