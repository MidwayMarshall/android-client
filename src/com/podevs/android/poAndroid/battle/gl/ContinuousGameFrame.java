...
public class ContinuousGameFrame extends Game implements InputProcessor {
    ...
    private BattleActivityBaked2 activity;

    public ContinuousGameFrame(CallBacks2 call) {
        this.callBack = call;
    ...
    private void calculateGUI() {
        ...
        battleAtlas = new TextureAtlas(Gdx.files.internal("data/sheets/battle3.txt"));

        font = new BitmapFont(Gdx.files.internal("data/sheets/battle.fnt"), battleAtlas.findRegion("font"));
        ...
        if (!path.equals("0")) {
            if (side) {
                ...
                if (spriteAtlas[me].findRegion("001") == null) {
                    ...
                    for (int i = 1; i < spriteAtlas[me].getRegions().size; i++) {
                        regions.add(spriteAtlas[me].findRegion(String.format("%03d", i)));
                    }
                }
                ...
                if (spriteAtlas[opp].findRegion("001") == null) {
                    ...
                    for (int i = 1; i < spriteAtlas[opp].getRegions().size; i++) {
                        regions.add(spriteAtlas[opp].findRegion(String.format("%03d", i)));
                    }
                }
    ...
    public void setBackground(int id) {
        background = new Image(new Texture(Gdx.files.internal("data/background/" + id + ".png")));
        background.setWidth(width);
        background.setHeight(height);
    }

    private CallBacks2 callBack = null;
    public interface CallBacks2 {
        BattleActivityBaked2 hook();
        void callForward(ContinuousGameFrame frame);
    }
